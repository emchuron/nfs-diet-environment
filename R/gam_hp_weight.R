gam_hp_weight<-function (mod, type = "dev", commonality = FALSE) 
{
  if (!inherits(mod, c("glm", "lm", "gam"))) 
  ...
  if (inherits(mod, c("gam", "glm", "lm"))) {
    ...
    colnames(dat)[7]<-"N" # Fixes error
    if (!inherits(dat, "data.frame")) {
      stop("Please change the name of data object in the original gam analysis then try again.")
    }
    if ("gam" %in% class(mod)) 
      to_del <- paste(as.character(mod$formula)[2], "~", 
                      "1")
    else to_del <- paste(as.character(mod$call$formula)[2], 
                         "~", "1")
    mod_null <- stats::update(object = mod, formula. = to_del, 
                              data = dat)
  }
  outputList <- list()
  outputList[[1]] <- outr2
  for (k in 1:nr2type) {
    ...
    for (i in 1:totalN) {
      ...
      to_add <- paste("~", paste(tmp.name, collapse = " + "), 
                      sep = " ")
    ...
    for (i in 1:totalN) {
      outputcommonM[i, 1] <- round(commonM[commonM[i, 1], 
                                           3], digits = 4)
